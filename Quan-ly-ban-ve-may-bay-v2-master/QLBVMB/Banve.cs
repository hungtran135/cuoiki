...
namespace QLBVMB
{
    public partial class Banve : Form
    {
        ...
        public DataTable getHANHLI(string mahang)
        {
            SqlConnection con = dc.GetConnect();


            string sql = string.Format("SELECT * FROM HANHLI WHERE MAHHK='{0}' ", mahang);
        ...
        public string get_GIA_HANHLI(string mahang, string tenhl)
        {
            SqlConnection con = dc.GetConnect();


            string sql = string.Format("SELECT * FROM HANHLI WHERE MAHHK='{0}' and TENHL='{1}' ", mahang, tenhl);
        ...
        public string get_MA_HANHLI(string mahang, string tenhl)
        {
            SqlConnection con = dc.GetConnect();
        ...
        private void button_Muave_Click(object sender, EventArgs e)
        {
            ...
            if (textBox_MaCB.Text.Trim() != "" && textBox_CMND.Text.Trim() != "" && textBox_TenKH.Text.Trim() != "" && textBox_SDT.Text.Trim() != "" && comboBox_hv.Text.Trim() != "")
            {
                try
                {
                    ...
                    decimal gia = Convert.ToDecimal(textBox_gia.Text) + Convert.ToDecimal(textBox_gia_hanh_li.Text);
        ...
        private void textBox_getMV_TextChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                button_chitiet.BackgroundImage = Properties.Resources.toolBanVe;
            }
        }
        ...
        private void checkBox_hanhli_CheckedChanged(object sender, EventArgs e)
        {
            //if(checkBox_hanhli.Checked == true)
            //{
            //    comboBox_hanhli.Enabled = true;
            //}
        }

        private void textBox_hanghk_TextChanged(object sender, EventArgs e)
        {
            if (textBox_hanghk.Text == "")
            {
                comboBox_hanhli.Enabled = false;
                textBox_gia_hanh_li.Enabled = false;

            }
            else
            {
                comboBox_hanhli.Enabled = true;
                textBox_gia_hanh_li.Enabled = true;
            }


            comboBox_hanhli.SelectedIndex = 1;
            string selected = comboBox_hanhli.Text; //this.comboBox_hanhli.GetItemText(this.comboBox_hanhli.SelectedItem);
            textBox_gia_hanh_li.Text = get_GIA_HANHLI(textBox_hanghk.Text, selected);

            //DataTable dtHanhLi = new DataTable();

            //dtHanhLi = getHANHLI(textBox_hanghk.Text);



            //comboBox_hanhli.DataSource = dtHanhLi;
            //comboBox_hanhli.ValueMember = "MAHL";
            //comboBox_hanhli.DisplayMember = "TENHL";





            //comboBox_hanhli.SelectedValue = 1;
            //


        }

        private void comboBox_hanhli_SelectedIndexChanged(object sender, EventArgs e)
        {



            // textBox_gia_hanh_li.Text = get_GIA_HANHLI(textBox_hanghk.Text, comboBox_hanhli.SelectedValue.ToString());


        }

        private void comboBox_hanhli_TextChanged(object sender, EventArgs e)
        {

        }

        private void comboBox_hanhli_SelectionChangeCommitted(object sender, EventArgs e)
        {
            string selected = comboBox_hanhli.Text; //this.comboBox_hanhli.GetItemText(this.comboBox_hanhli.SelectedItem);
            textBox_gia_hanh_li.Text = get_GIA_HANHLI(textBox_hanghk.Text, selected);
